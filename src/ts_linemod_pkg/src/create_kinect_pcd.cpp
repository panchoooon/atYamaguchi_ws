...
// PCD出力先
const char* OUTPUTPATH = "/home/guest/athome_ws/src/yamaguchi_pkg"; //PATHはpcに合わせて変更
...
public:
    CreateTemplate(){
        ...
        pub_pt0_ = nh_.advertise<sensor_msgs::PointCloud2>("/obj/pt0", 1); //オリジナル点群保存用
        //pub_pt1_ = nh_.advertise<sensor_msgs::PointCloud2>("/obj/pt1", 1);
        std::cout << "After pub_pt0_" << std::endl;
    ...
    void subFuncSave(const sensor_msgs::PointCloud2ConstPtr &in_cloud_const_ptr){
        std::cout << "Enter subFuncSave" << std::endl;
        ...
        pcl::PCDWriter writerTest; //18/04/17 Yamaguchi
        writerTest.write<pcl::PointXYZRGBA> ("test1.pcd", *in_cloud_ptr); //18/04/17 Yamaguchi

        // for visualization
        pcl::PointCloud<pcl::PointXYZRGBA>::Ptr proc0_cloud_ptr(new pcl::PointCloud<pcl::PointXYZRGBA>); //オリジナル点群出力用
...
//今回はロボットが観た点群ではなくKinect単体なので消す．2018/04/17 by Yamaguchi
/*
        // pass through
...
//セグメンテーションするので平面検出は必要ない 2018/04/17 by Yamaguchi
/*
        // plane detection
...
*/        
        // save object cloud
        cv::imshow("type 1 to save.", *tmpimg_); //ウィンドウが立ち上がる
        int key = cv::waitKey(30); 
        if (key == 49){ //49番のキー(sっぽい)を押したら
            ROS_INFO("save:%d", num_savefile_);
            char str_ent[100]; // オリジナル点群を保存する用
            char str_obj[100]; // 平面除去後の点群を保存する用
            snprintf(str_ent, 100, "%s/ent_%d.pcd", OUTPUTPATH, num_savefile_);
            snprintf(str_obj, 100, "%s/obj_%d.pcd", OUTPUTPATH, num_savefile_);
            pcl::io::savePCDFileBinary(str_ent, *proc0_cloud_ptr); //オリジナル点群出力用 
...
int main(int argc, char** argv){
    ros::init(argc, argv, "create_kinect_pcd");
